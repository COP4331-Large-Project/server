...
const createToken = (payload, expiresIn = defaultExpireTime) => jsonwebtoken_1.default.sign(payload, process.env.JWT_SECRET, { expiresIn });
...
const prepareToken = (req, next) => {
    ...
    if (!token) {
        return next(new APIError_1.default('Invalid authorization header', 'Bad authorization header given', 403));
    }
    ...
    try {
        ...
        if (err.name === 'TokenExpiredError') {
            return next(new APIError_1.default('Invalid JWT token', 'The given token has expired', 403));
        }
        return next(new APIError_1.default('Invalid JWT token', 'The given token failed to verify as a valid token', 403));
    }
};
...
const authenticate = (req, res, next, toCheck) => {
    ...
    if (decoded[key] !== toCheck[key]) {
        return next(new APIError_1.default('Not authenticated', 'The user is not authenticated for this action', '403'));
    }
    return next();
};
